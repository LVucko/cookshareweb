...
const RecipeEdit = () => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.put('/api/recipes', {id: id, title: title, shortDescription: shortDescription, longDescription: longDescription, categories: selectedCategories, pictureIds: pictureIds}, 
    ...
    return (
        <div className = "register">
            ...
            <form onSubmit = {handleSubmit}>
                ...
                {recipeCategories && allCategories && allCategories.map((category) =>(
                    <div className= "categories" key = {category.name}>
                        <label htmlFor={category.name}>
                            <input type="checkbox" defaultChecked={recipeCategories.includes(category.name)} id={category.id} name={category.name} value={category.id} onChange={(event) => { handleCheckboxChange(event) }}></input>
                            {category.name}
                        </label>
                    </div>
                ))}
                ...
                {isProcesing && <button id="disabledButton" disabled={true}>Izmjeni recept</button>}
            </form>
        </div>
